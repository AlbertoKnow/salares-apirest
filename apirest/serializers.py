...
class SharedFilesSerializer(ModelSerializer):
    class Meta:
        model=Files
        exclude=['share_files']
...
class FilesSerializer(ModelSerializer):
    ...
    share_files = SharedFilesSerializer(many=True, required=False)
    class Meta:
        model=Files
        fields='__all__'
'''
    def update(self, instance, validated_data):
        share_file_data = validated_data.pop('share_files')
        share_file = instance.share_file

        instance.name = validated_data.get('name', instance.name)
        instance.description = validated_data.get('email', instance.email)
        instance.save()

        profile.is_premium_member = share_file_data.get(
            'is_premium_member',
            profile.is_premium_member
        )
        profile.has_support_contract = share_file_data.get(
